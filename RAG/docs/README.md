...
| `content` | String | Текст чанка; для `table_row` — JSON-объект `{"колонка": "значение"}` |
...
| `table_row` | Одна строка таблицы: `content` = JSON-объект колонка-значение, `table_headers` = JSON-массив колонок (порядок) |
| `table_full` | Таблица целиком текстом |
| `table_raw` | Таблица, которую не удалось разобрать, — как есть |

### Почему строка таблицы хранится объектом

`{"Имя": "srv-app-01", "IP-адрес": "10.20.30.41", "Роль": "Сервер приложений"}`

Строка самодостаточна: имена колонок попадают в текст эмбеддинга (иначе вектор
строки состоит из голых значений без всякого контекста), находятся через
`exact_search` и не требуют join с `table_headers` при чтении. Объём это
увеличивает примерно на 70%, но текст — единицы мегабайт против ~150 МБ векторов
(1024 float32 = 4 КБ на чанк), поэтому цена незначительна. Хранить ту же строку
вторым чанком в другом формате смысла нет: это удвоило бы количество векторов и
давало бы дубли в выдаче. Двойное представление уже обеспечивают
`table_full` (контекст) и `table_row` (точность).
...
схлопнулась бы в один ключ с потерей ячеек. Если первая строка таблицы — сплошная
объединённая ячейка-заголовок, а следующая похожа на шапку (короткие различающиеся
значения), названия колонок берутся из неё.
