...
# MCP Server (Streamable HTTP, инструменты по MCP-протоколу)
.\start_kb_tools_mcp_http.ps1
# Статус: http://localhost:8000/health  |  MCP endpoint: http://localhost:8000/mcp
...
| `kb_tools_mcp_http.py` | **MCP-сервер** (Streamable HTTP) — все 16 инструментов по протоколу MCP | Подключение MCP-клиентов (Continue.dev, Claude Code и др.) по HTTP |
...
| `kb_tools_mcp_http.py` | MCP-сервер (Streamable HTTP, официальный MCP SDK) — публикует инструменты по протоколу MCP |
| `kb_tools_mcp_stdio.py` | MCP-сервер (stdio-транспорт) для локальных клиентов вроде Continue.dev |
...
Все 16 инструментов базы знаний публикуются по протоколу **MCP** (Model Context Protocol)
на официальном SDK `mcp`. Доступны два транспорта:
...
```powershell
# Запуск через uv (использует .venv проекта)
...
- Статус: `GET /health` -> `{ "status": "ok", "tools_count": 16, ... }`
...
```powershell
# Запущенный HTTP-сервер: /health + MCP handshake + вызов инструмента
.\test_mcp_server.ps1
```

**Подробнее:** [QUICKSTART_MCP.md](QUICKSTART_MCP.md), [MCP_SERVER.md](MCP_SERVER.md).
