...
`src_is_cidr` / `dst_is_cidr`), `protocol`, `port_from` / `port_to`, `service`,
`rule_desc`, а также исходные `src_text` / `dst_text` / `ports_text` и координаты
в базе знаний (`source`, `section`, `chunk_index`, `row_no`). Повторный `build`
идемпотентен: `flow_id` — устойчивый UUID5 от состава потока.
